...
export default function CreateSchedulerPage() {
  ...
  const generateCronExpression = () => {
    ...
    // 매일이 선택되어 있거나 모든 요일이 선택된 경우
    const days = selectedDays.includes("*") || selectedDays.length === 7 
      ? "*" 
      : selectedDays.join(",");
    
    // 여러 시간이 있는 경우 첫 번째 시간만 사용 (실제로는 여러 스케줄러를 생성해야 함)
  ...
  const handleSubmit = async (e: FormEvent) => {
    ...
    try {
      ...
      console.error("스케줄러 생성 중 오류가 발생했습니다:", error);
      alert("스케줄러 생성 중 오류가 발생했습니다.");
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <div className="p-6">
      <div className="flex items-center justify-between mb-6">
        <h2 className="text-xl font-semibold text-gray-900">새 스케줄러 생성</h2>
        <Link
